...
def aplicar_estilo_visual():
  # Fundo embutido diretamente em base64 (SVG proprio, gerado para esta app) —
  # evita depender de um link externo (GitHub) que pode falhar a carregar.
  url_imagem = "https://raw.githubusercontent.com/Rsousa1213/Analise-desportiva-2026/main/fundo.jpg"
  st.markdown(
      f"""
        <style>
        .stApp {{
            background-image: linear-gradient(rgba(0, 0, 0, 0.35), rgba(0, 0, 0, 0.55)), url('{url_imagem}') !important;
...
# --- Constantes do modelo (antes "magic numbers" espalhados pelo código) ---
...
def rho_correction(gh, ga, lam_h, lam_a, rho):
  """Fator de correção de Dixon-Coles para os 4 marcadores baixos.

  O modelo de Poisson puro assume que os golos da equipa da casa e da
  equipa visitante são independentes. Na prática, jogos com 0-0, 1-0, 0-1
  e 1-1 acontecem com uma frequência ligeiramente diferente da prevista
  por Poisson independente. Este ajuste corrige só esses 4 marcadores.
  """
  if gh == 0 and ga == 0:
...
def media_com_shrinkage(valores, pesos, media_liga, n_min=JOGOS_MIN_PARA_CONFIANCA_TOTAL):
  """Média ponderada da equipa, "puxada" para a média da liga quando há poucos jogos."""
  n = len(valores)
  if n == 0:
    return media_liga
...
def calcular_lambdas_mercado(df_liga, home_team, away_team, col_pro_casa, col_pro_fora, floor=0.4):
  """Modelo genérico ataque/defesa (usado para golos, cantos e cartões).

  col_pro_casa: coluna que mede o que a equipa da casa produz nesse mercado (ex: FTHG, HC, HY)
  col_pro_fora: coluna que mede o que a equipa visitante produz nesse mercado (ex: FTAG, AC, AY)

  Nota: nesta versão a defesa de cada equipa é normalizada pela média da liga
  na MESMA escala/coluna que o valor sofrido (ex: o que a equipa da casa sofre
  em casa é medido pela coluna "fora", e por isso é normalizado pela média da
  liga dessa mesma coluna "fora" — e não pela coluna "casa", como acontecia
  antes por engano, o que estava a introduzir um viés sistemático no cálculo).
  """
...
# --- Persistência do histórico de apostas (SQLite local, sem custos) ---
def inicializar_bd():
  ...
  """)
  # Migração segura: adiciona as colunas novas a bases de dados já existentes
  # (bases criadas antes desta versão não têm casa_aposta nem odd_fecho).
...
with tab_analise:
  ...
  if home_team == away_team:
    ...
    # Golos, cantos e cartões usam agora o mesmo modelo genérico de ataque/defesa
    ...
    # Matriz de Poisson com correção de Dixon-Coles nos marcadores baixos
    max_goals = 6
